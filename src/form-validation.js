  ...
  const onSubmitForm = (event) => {
    ...
    for (let f of inputFile.current.files) {
      if (f.size > maxSize * 1000) {
        alert(`ขนาดของแต่ละไฟล์ไม่เกิน ${maxSize}`);
        event.preventDefault();
        return;
      }
    }
    ...
    >
      <form onSubmit={handleSubmit(onSubmitForm)}>
        <div className="form-group mb-3">
          <label htmlFor="cats" className="form-label">
            หมวดหมู่ *
          </label>
        ...
        <div className="form-group mb-3">
          ...
          />
          {errors.name && (
            <div style={err}>กำหนดราคาสินค้าเป็นตัวเลขมากกว่า 0</div>
          )}
        </div>
        <div className="form-group mb-3">
          ...
          ></textarea>
          {errors.name && <div style={err}>ต้องกำหนดรายละเอียดสินค้า</div>}
        </div>
        <div className="form-group mb-3">
